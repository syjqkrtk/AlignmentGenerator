...
%             disp(size(result{i}));
    ...
    for i = 1:100
        ...
        if accuracy < temp2
            a = sum((count>=i).*(class==this))/sum(class==this,1);
            b = sum((count<i).*(class~=this))/sum(class~=this,1);
            c = (sum((count>=i).*(class==this))+sum((count<i).*(class~=this)))/1766;
            accuracy = temp2;
            threshold2 = i;
        %else
            %break;
    ...
    answer = [accuracy threshold2 a b c];
end
